...
public class PoeNinjaClient : IPoeNinjaClient
{
    ...
    private async Task<IEnumerable<NinjaPrice>> FetchItems(ItemType itemType)
    {
        ...
        try
        {
            ...
            var result = await responseStream.FromJsonToAsync<PoeNinjaQueryResult<PoeNinjaItem>>();
            if (result == null)
            {
                return [];
            }
    ...
    private async Task<IEnumerable<NinjaPrice>> FetchCurrencies(ItemType itemType)
    {
        ...
        try
        {
            ...
            var response = await client.GetAsync(url);
            var responseStream = await response.Content.ReadAsStreamAsync();
            var result = await responseStream.FromJsonToAsync<PoeNinjaQueryResult<PoeNinjaCurrency>>();
            if (result == null)
            {
                return [];
            }
